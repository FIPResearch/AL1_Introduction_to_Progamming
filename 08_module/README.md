...
~~コード [`08_module_set.py`](08_module_set.py) を眺めましょう．~~
~~このコードは、別のコードのモジュール（[`08_module_calc.py`](08_module_calc.py)）を利用し、そのモジュールをimportして実行してみてください．~~

> ~~ヒント：モジュールを外部からのパッケージimportすると、モジュールに含まれるすべての関数が import され、`モジュール名.関数名` で使うのですが、特定の関数だけimportすることも可能です．~~
...
`チャレンジ課題4`のコードをさらに書き直し，コード[`08_module_def3.py`](08_module_def3.py)をimportして、モジュールに別名をつけて、`チャレンジ課題4`コードの最後の行にimportしたモジュール`finalcall()`の関数を実行します.
