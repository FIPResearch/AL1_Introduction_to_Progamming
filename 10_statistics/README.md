...
さて，[前章のヒストグラム](../09_prob_plot)を見て，点数の分布が山なり形状になっているように感じます．
...
コード [`linear_regression.py`](10_linear_regression.py) はみかんの収穫量に対して直線を当てはめて回帰分析を適用した結果です．
...
コード [`10_linear_regression.py`](09_linear_regression.py) を実行し，**線形回帰**(linear regression)を適用したときのみかん収穫量の予測結果を確認せよ．<br>
...
コード [`10_polynomial_regression.py`](10_polynomial_regression.py)の多項式次数(degree)を増加させたときに，
R2スコアがどう変化するか，そして2023年，2030年の収穫量の予測結果がどう変化するかを確認せよ．

### 練習問題 18：様々な回帰モデル

コード [`10_polynomial_regression2.py`](10_polynomial_regression2.py)を見てみましょう．
このコードは1946年～2020年の消費者物価指数のデータ([`bukka.csv`](bukka.csv))に対して多項式回帰を適用したものです．下に，年次ごとに消費者物価指数をプロットしたものを示します．
...
- 線形回帰
- 多項式回帰 (degreeで`2次以上の関数`を決める)

> [`bukka.csv`](bukka.csv)は「[持家の帰属家賃を除く総合指数（1947年～最新年）](https://www.e-stat.go.jp/stat-search/files?tclass=000001138366)」（総務省）を加工して作成

### 練習問題 19：リッジ回帰モデル

練習問題 18 の元に**リッジ回帰**を示します．自身で調べてみて，活用してみましょう．

>　ヒント： リッジ回帰 (`sklearn.linear_model.Ridge()`)

---
[< 前へ](../09_prob_plot) | [トップに戻る](https://github.com/FIPResearch/AL1_Introduction_to_Programming) | 
